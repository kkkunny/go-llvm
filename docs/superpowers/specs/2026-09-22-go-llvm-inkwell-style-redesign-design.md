...
| §5 P2-3 属性与调用约定 | root `Attribute`/`AttributeKind`/`CallConv`/`AttrIndex`；`Function`/`Param`/`Call`/`Invoke` 的增删查属性与调用约定。LLVM-C 的函数与调用点属性 API 分开（`LLVM*AttributeAtIndex` vs `LLVM*CallSite*Attribute`），`ir` 内部按调用点标志分流。枚举属性 kind 由 `AttributeKindForName` 动态解析（LLVM 22 无 `argmemonly` 等旧名）。C API 未暴露属性组（attribute group），以函数/参数/返回/调用点四类位置覆盖 |
| §5 P2-5 metadata/comdat/inline asm/blockaddress | root `Metadata`（`MDString`/`MDNode`/`ValueAsMetadata`、`ModuleFlagBehavior`）、`InlineAsmDialect`/`Context.InlineAsm`、`ComdatSelectionKind`；`ir` 提供命名元数据、模块 flag、指令元数据附着（要求 MDNode）、`BlockAddress` 与 `Comdat` 角色。`Builder.Call` 支持 InlineAsm 调用（经 `LLVMGetInlineAsmFunctionType` 取签名，`LLVMGetFunctionType` 对非 Function 会断言） |
| §5 P2-6 诊断/Link/ctors/pass | `Context.SetDiagnosticHandler`/`ClearDiagnosticHandler`（`ErrorHandling.c` trampoline + Go 注册表；LLVM 默认 handler 对 error 会 `exit(1)`）。`Module.Link` 同 Context 前置校验，源模块被 LLVM 消费、Go 侧句柄立即失效（跨 Context 链接会产出非法 IR，故拒绝）。`Module.AppendCtor/AppendDtor` 以重建 appending 全局实现。`llvm/pass`：`RunPasses`/`RunPassesOnFunction`/`AutoOpt` + `Option` 选项，失败归 `ErrPass`（新增 `ErrLink`/`ErrPass`） |
